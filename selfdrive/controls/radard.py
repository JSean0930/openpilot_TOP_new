...
# -------------------- 視覺優先：新增的兩個門檻（差異最小） --------------------
# [ADDED] 視覺最低採用門檻：只要 prob 大於此值，就先用純視覺生成 lead
VISION_PROB_MIN = 0.35
# [ADDED] 雷達覆蓋上限：若視覺 prob 高於此值，就不讓雷達覆蓋（仍可低速補強按原規則覆蓋）
...
def get_lead(v_ego: float, ready: bool, tracks: dict[int, Track], lead_msg: capnp._DynamicStructReader,
  ...
  # [CHANGED] 決策順序改為「視覺優先（prob>VISION_PROB_MIN 直接用純視覺）→ 雷達匹配可在視覺較弱時覆蓋 → 低速保守補強」
