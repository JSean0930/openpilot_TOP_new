...
RADAR_TO_CENTER = 2.2 #2.7   # (deprecated) RADAR is ~ 2.7m ahead from center of car
...
def get_lead(v_ego: float, ready: bool, tracks: dict[int, Track], lead_msg: capnp._DynamicStructReader,
  ...
  """
  速度自動切換策略：
    - v_ego > 60 km/h：使用「原版：雷達 + 視覺融合（雷達為主）」邏輯
    - 其餘速度：使用「差異最小的視覺優先」邏輯
  低速保守覆蓋（low_speed_override）保留原行為。
  """
  # --- 高速：退回原版（雷達為主） ---
  if v_ego > SPEED_SWITCH_MS:
    ...
    if track is not None:
      ...
      # 雷達無匹配 → 退為純視覺（原版 fallback）
      lead_dict = get_RadarState_from_vision(lead_msg, v_ego, model_v_ego)

    # 低速覆蓋：高速邏輯下仍保留，但高速通常不會命中條件
  ...
  # --- 非高速：差異最小的視覺優先 ---
  # 決策順序：「視覺優先（prob>VISION_PROB_MIN 直接用純視覺）→ 雷達可在視覺較弱時覆蓋 → 低速保守補強」
  lead_dict = {'status': False}

  # 1) 視覺優先：只要 ready 且 prob 過低門檻，就先用視覺生成 lead
  ...
  # 3) 低速保守覆蓋（完全沿用原本邏輯）：若符合條件，選最近的低速雷達點覆蓋
  if low_speed_override:
    low_speed_tracks = [c for c in tracks.values() if c.potential_low_speed_lead(v_ego)]
    if len(low_speed_tracks) > 0:
      closest_track = min(low_speed_tracks, key=lambda c: c.dRel)
