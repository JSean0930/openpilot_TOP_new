...
- Toyota 上緣：低速靈敏、高速平緩；封頂 130 km/h；線性分段
- 煞車下限：回復到通用下限（ACCEL_MIN），不再用 Toyota 專屬煞車表
- 檔頭布林開關：
  * E2E_ONLY_ENABLED        ：純 E2E 主導（不混用 MPC 的 a_target）
  * E2E_ONLY_STRICT_LIMITS  ：E2E-only 是否保留 Toyota 上緣 + 轉彎限縮（安全版）
  ...
  def update(self, sm):
    ...
    if mode_for_limits == 'acc':
      if self.CP.brand == "toyota":
        # 回復成「Toyota 只有上緣、下緣用通用 ACCEL_MIN」
    ...
    if self.e2e_only and E2E_MIN_GAP_LIMIT_ENABLED:
      ...
      if getattr(lead, "status", False):
        ...
        d_min = e2e_min_gap_required(v_ego)
        band  = max(0.5, float(E2E_MIN_GAP_BAND_M))
        d_err = d_rel - d_min
