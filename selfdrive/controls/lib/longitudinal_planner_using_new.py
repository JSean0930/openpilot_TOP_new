...
# -*- coding: utf-8 -*-
"""
LongitudinalPlanner（E2E-only 可切換 + 簡易 FCW 版）
- 保留原功能：ACC/Blended 模式、SNG 紅燈偵測、VTSC、油門允許、轉彎限縮
- Toyota 加/減速上下緣曲線：低速靈敏、高速平緩；封頂 130 km/h；全程線性
- 新增：
  1) Params: LongE2EOnly → 開啟後完全以 modelV2 主導（E2E-only），不再混用 MPC 輸出
  2) E2E-only 下的簡易 FCW（TTC/距離/逼近速度 + 輕微遲滯）
  3) 仍保留所有物理夾限（安全冗餘）
"""
...
class LongitudinalPlanner(LongitudinalPlannerTOP):
  ...
  def __init__(self, CP, init_v=0.0, init_a=0.0, dt=DT_MDL):
    ...
    # === E2E-only switch（Params: LongE2EOnly） ===
    try:
      self.e2e_only = self.params.get_bool("LongE2EOnly")
    except Exception:
      self.e2e_only = False
  ...
  def update(self, sm):
    ...
    5) VTSC 限速、（必要時）MPC 解算
    6) 輸出選擇：E2E-only 或 ACC/Blended
    7) 夾限輸出加速度、保存軌跡與 flags、FCW 判斷
    """
    ...
    # 個性化上限（若啟用 personality），覆蓋上緣並再套轉彎限制
    ...
    e2e_j_traj = np.zeros_like(CONTROL_N_T_IDX)  # 無 j 輸出，用 0 代替
    ...
    # ====== 準備 MPC（即使 E2E-only，也可保留運行以保有 crash_cnt 等診斷） ======
    ...
    # ====== 取 MPC 解並內插到 CONTROL_N（供 ACC/Blended 時使用） ======
    self.v_desired_trajectory = np.interp(CONTROL_N_T_IDX, T_IDXS_MPC, self.mpc.v_solution)
    self.a_desired_trajectory = np.interp(CONTROL_N_T_IDX, T_IDXS_MPC, self.mpc.a_solution)
    self.j_desired_trajectory = np.interp(CONTROL_N_T_IDX, T_IDXS_MPC[:-1], self.mpc.j_solution)

    # FCW（預設使用 MPC crash_cnt）；E2E-only 另採簡易 FCW
    ...
    if self.e2e_only:
      ...
      # 原行為：ACC 用 MPC；Blended 取 min(MPC, E2E) 並 OR 停車旗標
      if self.mode == 'acc':
